...
const RejectedJobs = () => {
  ...
  useEffect(() => {
    ...
    fetch('http://localhost:5000/company/companyRejected').then((res) => res.json())
        .then((data) => {
                  ...
                  {candidateData.map((data) => (
                    <tr className="border-b" key={data.id}>
                      ...
                      <td className="text-sm text-gray-900 font-light px-6 py-4 whitespace-nowrap flex gap-5">
                        <div className="flex gap-2 items-center font-bold bg-red-500 text-white p-2 rounded-md cursor-pointer">
                          Rejected
                        </div>
                        <div
